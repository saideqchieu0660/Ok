...
import { Play, BookOpen, Search, X, ChevronLeft, ChevronRight, Sparkles, Pin, PinOff, Clock, Check, Share2, Edit3, DownloadCloud, FileJson } from 'lucide-react';
...
const TiltCard = ({ children, delayIdx, className = "", onClick }: { children: React.ReactNode, delayIdx: number, className?: string, onClick?: () => void }) => {
  ...
  if (isFixLagEnabled) {
    return (
      <div className={`h-full content-visibility-auto gpu-accelerated ${className}`}>
        <div onClick={onClick} className="relative p-4 sm:p-5 rounded-2xl flex flex-col group overflow-visible min-h-[10rem] h-full border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 transition-colors shadow-sm cursor-pointer">
          {children}
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      <div onClick={onClick} className="card-3d relative p-4 sm:p-5 rounded-2xl flex flex-col group overflow-visible min-h-[10rem] h-full transform-style-3d bg-white/70 dark:bg-black/80 cursor-pointer">
        {children}
      </div>
    </motion.div>
  );
          ...
          groupBySubject ? (
                      ...
                      return (
                        <TiltCard key={`${deck.id || "deck"}-${idx}`} delayIdx={idx} onClick={() => navigate(`/study/${deck.id}`)} className={cn(
                          ...
                          <div className="absolute top-4 right-4 z-20 flex gap-2">
                            ...
                            <button
                              onClick={(e) => { e.preventDefault(); e.stopPropagation(); setClassModalDeckIds([deck.id]); }}
                              className="p-2 rounded-full transition-colors bg-orange-500/10 text-orange-600 dark:text-orange-400 hover:bg-orange-500/20"
                              ...
                              onClick={(e) => {
                                ...
                                navigator.clipboard.writeText(shareText).then(() => {
                                  toast.success("Đã sao chép link học phần!", {
                                    description: "Bây giờ bạn có thể dán (Paste) để chia sẻ cho học viên hoặc bạn học.",
                                  });
                                }).catch((err) => {
                                  console.error("Lỗi khi copy: ", err);
                                  toast.error("Không thể sao chép, vui lòng thử lại.");
                            ...
                            {(isAdmin || deck.createdBy === currentUser?.id) && (
                              <button
                                onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (onEditDeck) onEditDeck(deck); else setLocalEditingDeck(deck); }}
                                className="p-2 rounded-full transition-colors bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-500/20"
                            ...
                            <div className="relative">
                              <button
                                onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowDownloadMenu(showDownloadMenu === deck.id ? null : deck.id); }}
                                className="p-2 rounded-full transition-colors bg-purple-500/10 text-purple-600 dark:text-purple-400 hover:bg-purple-500/20"
                                title="Tải xuống JSON"
                              >
                                <FileJson className="w-5 h-5" />
                              </button>
                              {showDownloadMenu === deck.id && (
                                <div className="absolute top-full right-0 mt-2 bg-white dark:bg-zinc-800 rounded-xl shadow-xl border border-zinc-200 dark:border-zinc-700 py-2 w-48 z-50 flex flex-col overflow-hidden">
                                  <button 
                                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDownloadJson(deck, false); setShowDownloadMenu(null); }}
                                    className="px-4 py-2 text-left text-sm font-semibold hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200"
                                  >
                                    Tải toàn bộ bộ thẻ
                                  </button>
                                  <button 
                                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDownloadJson(deck, true); setShowDownloadMenu(null); }}
                                    className="px-4 py-2 text-left text-sm font-semibold hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200"
                                  >
                                    Chỉ tải thẻ đánh dấu X
                                  </button>
                                </div>
                              )}
                            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 sm:gap-10">
              {sortedAndFilteredDecks.map((deck, idx) => {
                ...
                return (
                  <div id={`deck-${deck.id}`} className="h-full" key={`${deck.id || "deck"}-${idx}`}><TiltCard delayIdx={idx} onClick={() => navigate(`/study/${deck.id}`)} className={cn("", isOfflineUnavailable && "opacity-40 grayscale pointer-events-none")}>
